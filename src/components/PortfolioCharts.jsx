...
export default function PortfolioCharts({ portfolioData, transactions = [], prices = {}, hideValues = false }) {
  ...
  const historyData = useMemo(() => {
    ...
    sortedTransactions.forEach((tx) => {
      ...
      if (tx.type.toLowerCase() === 'buy') {
        ...
        for (const buyTx of assetMap[ticker].transactions.filter(t => t.type.toLowerCase() === 'buy')) {
      ...
      const dateLabel = date.toLocaleDateString([], { month: 'short', day: 'numeric', year: date.getFullYear() !== new Date().getFullYear() ? 'numeric' : undefined });
    ...
    if (dataPoints.length > 0) {
      const currentCostBasis = portfolioData.reduce((sum, asset) => sum + asset.totalCost, 0);
      dataPoints.push({
        date: 'Now',
        value: totalValue,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
           ...
           <div className="flex items-center gap-2">
             ...
             {!hideValues && historyData.length > 1 && (
                 <span className={`text-sm font-bold ml-2 ${historicalTrend ? 'text-green' : 'text-red'}`}>
                   {historicalTrend ? '▲' : '▼'} {((Math.abs(historyData[historyData.length - 1].value - historyData[0].value) / historyData[0].value) * 100).toFixed(2)}%
                 </span>
             )}
           </div>
           {/* time period tabs */}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData}>
                ...
                <YAxis 
                  stroke="#52525b" 
                  tick={{ fill: '#71717a', fontSize: 10, fontWeight: 500 }}
                  tickFormatter={(value) => hideValues ? '****' : (value >= 1000 ? (value/1000).toFixed(1) + 'k' : value)}
                  tickLine={false}
                  axisLine={false}
                  orientation="right"
                  domain={['auto', 'auto']}
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-xl p-2.5 flex flex-col">
        <div className="mb-1">
            <h2 className="text-white font-bold mb-0.5 text-sm">Allocation</h2>
            <p className="text-[10px] text-[var(--text-secondary)]">Distribution by Asset Value</p>
        </div>
                ...
                {allocationData.map((entry, index) => (
                  <div key={entry.name} className="flex items-center justify-between text-xs">
                     <div className="flex items-center gap-1.5 overflow-hidden">
                        <div className="min-w-[8px] h-2 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)]" style={{ backgroundColor: COLORS[index % COLORS.length], boxShadow: `0 0 8px ${COLORS[index % COLORS.length]}` }}></div>
                        <span className="text-[var(--text-secondary)] truncate font-medium text-[11px]">{entry.name}</span>
                     </div>
